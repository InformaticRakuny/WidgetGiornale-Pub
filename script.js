...
// Memorizziamo la posizione iniziale di ogni giornale
const initialPositions = [
    'rotate(-15deg) translateX(-60px) translateY(-80px)', // Giornale 1
    'rotate(10deg) translateX(40px) translateY(-40px)',  // Giornale 2
    'rotate(-5deg) translateX(70px) translateY(50px)',   // Giornale 3
    'rotate(20deg) translateX(-30px) translateY(70px)'    // Giornale 4
];

// Funzione per gestire l'effetto di spostamento dei giornali
giornali.forEach((giornale, index) => {
    // Impostiamo la posizione iniziale per ogni giornale (come se fosse al caricamento della pagina)
